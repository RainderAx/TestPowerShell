...
# ce script gÈnÈre un fichier txt qui va Ítre traitÈ par un autre script en t‚che planifier


#Charge en m√©moire les √©l√©ments graphiques necessaires
...
#Valeurs qui permettent de v√©rifier les valeurs des champs necessaires pour cr√©er un utilisateur
...
$TextBureau.Add_LostFocus({
	...
	for ( $i=0; $i -lt ($TextBureau.text).Length; $i++ ) {
		...
		elseif ($TextBureau.Text.Length -ne 4) {
			ChangeLabelError $LabelBureauError 'Le champ doit contenir 4 caract√®res'
			return
		}
		else {
...
#Fonction qui permet de prendre les initials de plusieurs pr√©noms
...
#Fonction qui g√©n√®re un mot de passe al√©atoire
...
$button_generer.Add_Click({
	...
	[String]$Poste=$ComboBoxPoste.Get_text().Trim(' ') 
        ...
        Ce rapport a ÈtÈ gÈnÈrÈ par $utilisateur.

        Merci de vÈrifier si ces informations sont correctes" 
	...
	Catch {
		ChangeLabelError $LabelMessage 'La creation a √©chou√©. Voir les logs'
...
#D√©finit la valeur par d√©faut des listes d√©roulantes
