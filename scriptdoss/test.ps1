...
$button_generer.Add_Click({
    ...
    $test = ( Get-ADUser -Filter "samAccountName -like '$($Prenom)*$($Nom)*'").SamAccountName
    $userData = "$Global:bur,$test"
    $userData | Out-File -FilePath "C:\Scripts\Alexis\$test.txt"
